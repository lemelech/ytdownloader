...
COOKIES_PATH = Path(__file__).parent / "cookies.txt"  # exported from an incognito session, see README
...
def download_best_audio_as_mp3(video_url, save_path=SAVE_PATH): # got from https://dev.to/_ken0x/downloading-and-converting-youtube-videos-to-mp3-using-yt-dlp-in-python-20c5
    ...
    if COOKIES_PATH.exists():
        # avoids YouTube's bot check; see cookies.txt export steps in README
        ydl_opts['cookiefile'] = str(COOKIES_PATH)
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ydl.download([video_url])
    except DownloadError as e:
        if 'sign in' in str(e).lower() or 'cookie' in str(e).lower():
            print(
                f"Your YouTube cookies are missing or expired.\n"
                f"Re-export cookies.txt from an incognito YouTube session "
                f"(see README) and save it to:\n  {COOKIES_PATH}"
            )
        else:
            raise
    print(f"Done with {video_url}")
...
def ytdownloader(url, single=True):
    try:
        ...
        p = Process(target=download_best_audio_as_mp3, args=(download_url,  SAVE_PATH))
        p.start()
